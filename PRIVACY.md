...
- Local operation history such as file names, output paths, status, and timestamps.

This data is used only to provide the app's local functionality.

## Local Storage

Age Mac stores app state in the user's Application Support directory. Private keys remain local. Passphrases are used for the requested operation and are not intentionally written to operation history.
